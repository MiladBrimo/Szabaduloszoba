...
void setup()
{
  ...
  for (int i = 0; i < NR_OF_READERS-1; i++)
  {
    pinMode(REED[i], INPUT_PULLUP);
    pinMode(RELAY[i], OUTPUT);
    digitalWrite(RELAY[i], LOW);
  }
  pinMode(REED[3], INPUT); //Induktiv-erzekelo
...
void loop()
{
  if( !digitalRead(REED[0]) && !digitalRead(REED[1]) && !digitalRead(REED[2]) && digitalRead(REED[3]) ) //Induktiv-erzekelo
  {
